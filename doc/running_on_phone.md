...
> [!WARNING]
> Remember about switching off `Automatic date and time` function (in *Settings->System->Date and time*) otherwise Pure will hang/freeze after a while after starting debugging. Also remember to connect the USB to properly power the phone while debugging.
...
> [!NOTE]
> Running on phone is done via gdb, this way enables you to debug running target.
...
1. `pureflash`
    ...
    The fastest command to burn image into the eMMC card:

    ```bash
    sudo pureflash <PurePhone.img> /dev/sdX
    ```
...
2. `dd` or `etcher`
    ...
    > [!NOTE]
    > End-user devices have direct access to the eMMC locked in bootloader.
...
> [!NOTE]
> Providing a minimum viable update package and utility for the Open Source community is on our backlog. It can be created by trimming additional development API from:
...
> [!NOTE]
> all phones in field have eMMC access locked with a password, the following documentation is only for development devices or ones with eMMC unlocked or bypassed.
...
> [!NOTE]
> Devices in field have password-locked bootloaders. This is by design - normal users shouldn't ever require access to raw eMMC.
...
Both FAT and ext4 can be mounted natively on Linux, ext4 needs additional drivers on macOS. To access LittleFS you need to mount it via FUSE using the following command:

```bash
lfsfuse /dev/sdX <mount_point> --block_size=32768
```
